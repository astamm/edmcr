...
npf <- function(D,
    ...
    if (is.null(POptim)) {
        #If Dim Relax was unsuccessful, do a local minimization
    ...
    while (fPSol > toler) {
        ...
        POptim <- npfDimRelax(D,
                              ...
                              Method = method)
        if (is.null(POptim)) {
            #If DimRelax is unsuccessful, do a local minimization
        ...
        if ((fPSol - fPSolOld) / fPSolOld < sqrt(toler) |
            fPSol < toler) {
            break
        }
